...
Experiment.prototype.generateExperimentArguments = function(isCreate, isMetrics){
    ...
    if (theExperiment.attributes.metrics && isMetrics) {
        expArgs.metrics = theExperiment.attributes.metrics;
    } else if (isCreate) {
        //Requires a default metric
        expArgs.metrics = [{
            "aggregator": "sum",
            "field": "revenue",
            "scope": "visitor",
    ...
    variations.forEach(function(variationPath) {
        ...
        for (var i = 0; i < theExperiment.attributes.page_ids.length; i++) {
            var pageId = theExperiment.attributes.page_ids[i];
            var baseDir = variation.baseDir + '';
            variation.baseDir = path.join(variation.baseDir, String(pageId));
...
Experiment.prototype.createRemote = function(client, isMetrics) {
    var self = this;
    return client.createExperiment({}, self.generateExperimentArguments(true, isMetrics)).then(function(data) {
...
Experiment.prototype.updateRemote = function(client, isMetrics) {
    ...
    return client.updateExperiment({
        ...
        }, self.generateExperimentArguments(false, isMetrics)).then(function(data) {
            self.updateVariationIds(data);
